...
async def _dictionary(request: Request, dictionary_id: int, session: AsyncSession = Depends(get_session_depends)):
    ...
    return templates.TemplateResponse(
        "dictionary.html", {"request": request, "dictionary": DictionaryResponse.model_validate(dictionary)}
    )
...
async def _dictionary_update(
    ...
    await Dictionary.update(id=dictionary_id, name=body.name, session=session)
    return True
